...
const ScoreboardDisplay = ({ scoreboardData }) => {
  ...
  return (
    <div className="flex flex-col md:w-8/12 items-center justify-center bg-gray-400 p-8 rounded-3xl shadow-lg my-5">
      <div className="flex flex-col md:w-4/5 rounded-xlg">
        {scoreboardData.scoreboard.games.map((game, index) => (
          <div
            className="bg-blue-800 text-white text-center text-3xl font-bold rounded-2xl p-4 my-2 "
            key={index}
          >
            <div>{game.awayTeam.teamName} - {game.homeTeam.teamName}</div>
            <div>{game.awayTeam.score} - {game.homeTeam.score}</div>
